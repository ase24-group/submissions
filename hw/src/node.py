...
class Node:
    def __init__(self, data, left=None, right=None, C=None, cut=None, lefts=None, rights=None):
        self.here = data
        self.left = left
        self.right = right
        self.C = C
    ...
    def show(self):
        ...
        def _show(node, depth, leafp):
            post = (str(d2h(node.here)) + "\t" + utils.output(node.here.mid().cells)) if leafp else ""
            max_depth = math.max(max_depth, depth)
            print(('|..')*depth + post)

        self.walk(_show)
        print(("    ")*max_depth + str(d2h(self.here)) + utils.output(self.here.mid().cells))
